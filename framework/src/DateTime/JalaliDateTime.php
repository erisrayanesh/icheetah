...
class JalaliDateTime extends AbstractDateTime
{
    
    private $convert = false;
    private $temp = array();
    
    private $months = array ('فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور', 'مهر', 'آبان', 'آذر', 'دی', 'بهمن', 'اسفند');
    private $days = array('یکشنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه', 'شنبه');

    public function __construct($timestamp = null, $format = null, $timezone = "Asia/Tehran")
    {
        parent::__construct($timestamp, $format, $timezone);
        $this->setTimeZone("Asia/Tehran");
    }
    
    public static function create($timestamp = null, $format = null, $timezone = null)
    {
        return new static($timestamp, $format, $timezone);
    }
    
    public static function getNow($timestamp = null, $format = null, $timezone = null)
    {
        $instance = new static($timestamp, $format, $timezone);
        return $instance->now();
    }
    
    // ================ IDATETIME Interface METHODS ==========================
    
    /**
     * Returns formated date/time of current timestamp
     * @return string
     */
    public function now() 
    {
        return $this->jdate($this->format(), time(), "", $this->timezone(), "en");
    }
    
    public function tomorow()
    {
        return $this->jdate($this->format(), time() + 86400, "", $this->timezone(), "en");
    }
    ...
    /**
     * Returns an array of date/time info
     * @return array
     */
    public function dateInfo()
    {
        return $this->jgetdate($this->timestamp(), "" ,$this->timezone());
    }
    
    /**
     * Returns timestamp based on given arguments.
     * @param int $hour
     * @param int $minute
     * @param int $second
     * @param int $year
    ...
    {
        return $this->jmktime($hour, $minute, $second, $month, $day, $year);
    }
    
    /**
     * Returns formated date/time of timestamp
     * @return string
     */
    public function datetime()
    {
        return $this->jdate($this->format(), $this->timestamp(), "", $this->timezone(), "en");
    }
    ...
    public function add($interval)
    {
        ...
        } elseif (!is_a($interval, "DateInterval")) {
            return false;
        }
        
        $curDate = $this->jgetdate($this->timestamp(), "", $this->timezone());
        $timestamp = $this->makeTimestamp($curDate["hours"] + $interval->h, 
                                          $curDate["minutes"] + $interval->i, 
                                          $curDate["seconds"] + $interval->s, 
                                          $curDate["year"] + $interval->y, 
                                          $curDate["mon"] + $interval->m, 
                                          $curDate["mday"] + $interval->d);
        $this->setTimestamp($timestamp);
        return $this->jdate($this->format(), $timestamp, "", $this->timezone(), "en");
    }
    ...
    public function diff(AbstractDateTime $date)
    {
        if ($this->timestamp() > $date->timestamp()){
            $timestamp = $this->timestamp() - $date->timestamp();
        } elseif ($this->timestamp() < $date->timestamp()){
            $timestamp = $date->timestamp() - $this->timestamp();
        }
        
        $years = abs(floor($timestamp / 31536000));
        $months = abs(floor(($timestamp-($years * 31536000))/2592000));
        $days = abs(floor(($timestamp-($years * 31536000)-($months *  2592000 ))/86400));
        $hours = abs(floor(($timestamp-($years * 31536000)-($months *  2592000 )-($days * 86400))/3600));
        $minutes = abs(floor(($timestamp-($years * 31536000)-($months *  2592000 )-($days * 86400)-($hours * 3600))/60));#floor($difference / 60);
        $seconds = abs(floor(($timestamp-($years * 31536000)-($months *  2592000 )-($days * 86400)-($hours * 3600)-($minutes * 60))));
    ...
    public function toHijry()
    {
        
    }

    public function convertToFarsi()
    {
        return $this->convert;
    }

    public function setConvertToFarsi($convert)
    {
    ...
    private function jmktime($h = '', $m = '', $s = '', $jm = '', $jd = '', $jy = '', $is_dst = -1)
    {
        $h = $this->tr_num($h);
        $m = $this->tr_num($m);
        $s = $this->tr_num($s);
    ...
    private function jgetdate($timestamp = '', $none = '', $tz = 'Asia/Tehran', $tn = 'en')
    {
    ...
    private function jcheckdate($jm, $jd, $jy)
    {
        $jm = $this->tr_num($jm);
        $jd = $this->tr_num($jd);
        $jy = $this->tr_num($jy);
    ...
    private function gregorian_to_jalali($gy, $gm, $gd, $mod = '')
    {
        $gy = $this->tr_num($gy);
        $gm = $this->tr_num($gm);
        $gd = $this->tr_num($gd); /* <= Extra :اين سطر ، جزء تابع اصلي نيست */
